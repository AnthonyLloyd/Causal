...
module internal Statistics =
    ...
    let inline error (list:ListSlim<_>) (i:int) (j:int) (median:float) =
        let mutable s2 = 0.0
        for k = i to j do
            s2 <- s2 + sqr(float list.[k]-median)
        sqrt(s2/float((j-i+1)*(j-i))) * 1.253

    let inline mad (list:ListSlim<_>) (m:float) =
    ...
    let inline outliers (list:ListSlim<_>) =
        ...
        let mutable i = 0
        while float list.[i] < lowerBound do
            i <- i + 1

        let upperBound = m + 3.0 * mad
        let mutable j = count - 1
        while float list.[j] > upperBound do
            j <- j - 1
        i,j

    let inline estimate (list:ListSlim<_>) =
        let i,j = outliers list
        let median = median list i j
        Estimate(median, error list i j median)

module Seq =

    let medianNoOutliers (s:float seq) =
        let list = ListSlim()
        Seq.iter (list.Add >> ignore) s
        list.SortInPlace()
        let i,j = Statistics.outliers list
        Statistics.median list i j
